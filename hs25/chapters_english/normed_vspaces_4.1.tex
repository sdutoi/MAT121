...
\section*{4.1 Normed Vector Spaces}

In this section, we assume that the concept of the vector space is known from linear algebra and recall the fundamental definitions and properties.

\NumberedDefinition{4.1.1}{}{
A set $M$ with an operation $\circ : M \times M \to M$ is an \textit{Abelian Group}, if the operation possesses the properties:

(1) \textit{Commutative law}:
\[
a \circ b = b \circ a \quad \forall a,b \in M,
\]

(2) \textit{Associative law}:
\[
a \circ (b \circ c) = (a \circ b) \circ c \quad \forall a,b,c \in M,
\]

(3) \textit{Neutral element}:
\[
\exists e \in M \quad a \circ e = a \quad \forall a \in M,
\]

(4) \textit{Inverse element}:
\[
\forall a \in M \quad \exists a^{-1} \in M \quad a \circ a^{-1} = e.
\]
}

\NumberedDefinition{4.1.2}{}{
A set $X$ is called a $\K$-\textit{Vector Space}, if there are two mappings:
\begin{align*}
+ &: \quad X \times X \to X \quad \text{called "Addition"} \\
\cdot &: \quad \K \times X \to X \quad \text{called "(scalar) Multiplication"}
\end{align*}
with the following properties:

\textit{VS1} $(X, +)$ is an Abelian Group with neutral element $0$.

\textit{VS2} For all $\alpha, \beta \in \K$ and all $x, y \in X$ hold (\textit{the distributive laws}):
\begin{align*}
...
\textit{VS3} $1 \cdot x = x$.
}

\NumberedExample{4.1.3}{}{
...
\NumberedDefinition{4.1.4}{Norm}{Let $X$ be a $\K$-vector space. A map $\|\,\cdot\,\| : X\to\R$ is called a norm if the following hold:
...
\NumberedRemark{4.1.5}{}{From the triangle inequality one obtains the reverse triangle inequality
\[
	\|x\| - \|y\| \le \|x-y\|,
\]
...
\NumberedExample{4.1.6}{}{\begin{enumerate}[label={(\arabic*)}, leftmargin=*]
...
\end{enumerate}}
	...
	@4, 5: (N1), (N2) follow from the property (N1), (N2) on $\K$ and the calculation rules for series.
(N3) Use definition $\|x\|_2 := \sqrt{\sum_{n=0}^\infty |x_n|^2}$. Since $\sqrt{\sum_{k=0}^n |x_k|^2}$ is a norm on $\K^{n+1}$ (see (2)), it holds:
\[
\underbrace{\sqrt{\sum_{k=0}^n |x_k + y_k|^2}}_{:= a_n} \le \underbrace{\sqrt{\sum_{k=0}^n |x_k|^2} + \sqrt{\sum_{k=0}^n |y_k|^2}}_{:= b_n}
\]

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{balls.png}
    \caption{(Outer boundaries of) Balls in $\R^2$ with respect to different norms: left: $\ell_1$-norm, middle: $\ell_2$-norm, right: $\ell_\infty$-norm.}
\end{figure}

\noindent $(a_n), (b_n)$ are convergent sequences with $a_n \le b_n$ for all $n \in \N$. From this follows:
\[
\sqrt{\sum_{k=0}^\infty |x_k + y_k|^2} \le \sqrt{\sum_{k=0}^\infty |x_k|^2} + \sqrt{\sum_{k=0}^\infty |y_k|^2}
\]
\qed

	\NumberedRemark{4.1.7}{Convention}{In the following, $(X,\|\cdot\|)$ will always denote a normed vector space.}

	\NumberedDefinition{4.1.8}{Open ball}{For $x_0\in X$ and $r\in\R$ set
	\[
	...
	\NumberedRemark{4.1.9}{}{In a normed vector space one defines the concepts of sequence, convergent sequence, accumulation point, Cauchy sequence, and series analogously by replacing the absolute value $|\cdot|$ everywhere by the norm $\|\cdot\|$.

	\textbf{Convergence:} Let $(x_n)_{n\in\N}\subset X$ be a sequence with elements $x_n\in X$. Then $(x_n)$ converges to $x\in X$ precisely when
	\[
	...
	\NumberedDefinition{4.1.10}{Equivalent norms}{Let $X$ be a $\K$-vector space and let $\|\cdot\|_a,\ \|\cdot\|_b$ be two norms on $X$. They are called \emph{equivalent} if there exist positive constants $c$ and $\bar c$ such that
	\[
		c\,\|x\|_a \le \|x\|_b \le \bar c\,\|x\|_a \qquad \forall\, x\in X.
	\]}

	\NumberedRemark{4.1.11}{}{\begin{enumerate}[label={(\arabic*)}, leftmargin=*]
	...
	\NumberedTheorem{4.1.12}{On $\K^{n}$ all norms are equivalent}{Let $n\in\N$. On $\K^{n}$ all norms are equivalent.}
	\paragraph{Proof.}
	...
	\]
	hence $\|x^{*}\|=0$ and therefore $x^{*}=0$. 
	
	\noindent On the other hand:
\[
\|x^*\|_1 = \|x_{k_l} + (x^* - x_{k_l})\|_1 \ge \underbrace{\|x_{k_l}\|_1}_{\to 1} - \underbrace{\|x^* - x_{k_l}\|_1}_{\to 0} \xrightarrow{\ell \to \infty} 1
\]
This results in a contradiction to $x^* = 0$ and it holds that $\rho > 0$.

Let now $x \in \K^n \setminus \{0\}$ and $y := x/\|x\|_1$. Then $\|y\|_1 = 1$ follows and therefore
\[
\frac{\|x\|}{\|x\|_1} = \|y\| \in M.
\]
Thus holds
\[
\frac{1}{\|x\|_1} \|x\| \ge \rho > 0 \implies \rho \|x\|_1 \le \|x\|.
\]
This obviously also holds for $x = 0$, and thus it is shown that $\underline{c} = \rho$ can be chosen. \hfill $\blacksquare$

	\NumberedRemark{4.1.13}{}{Theorem~4.1.11 does not hold in infinite-dimensional vector spaces in general. For example, the norms $\|\cdot\|_\infty$ and $\|\cdot\|_1$ on $\ell_1$ are not equivalent.}
	\paragraph{Proof.}
	...
	\NumberedDefinition{4.1.14}{Complete (Banach) space}{A normed $\K$-vector space $(X,\|\cdot\|)$ is called \emph{complete} or a \emph{Banach space} if every Cauchy sequence in $X$ converges.}

	\NumberedTheorem{4.1.15}{The following normed $\K$-vector spaces are complete}{\begin{enumerate}[label={(\arabic*)}, leftmargin=*]
		\item $\K$,
		\item $\K^{n}$ for $n\in\N$,
